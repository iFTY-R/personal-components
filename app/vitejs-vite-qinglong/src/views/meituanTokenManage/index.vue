...
import { getMeituanTokens, QINGLONG_TASK_MEITUAN_ID, runMeituanTask, updateEnv } from "@/api/qinglong.js";
  ...
  proxyConfig: {
    ajax: {
      ...
      query({ page, sort, sorts, filters, form }) {
        refTaskLogDialog.value.getLogByTaskId(QINGLONG_TASK_MEITUAN_ID);
...
const handleGetTaskLog = () => {
  refTaskLogDialog.value.toggleDialog(true, QINGLONG_TASK_MEITUAN_ID);
}
